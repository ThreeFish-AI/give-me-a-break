...
> macOS 菜单栏强制作息应用——在自定义工作时段内执行「工作 / 强制休息」节律，休息时**遮罩全部显示器**并**控制 QQ 音乐**播放舒缓音乐；接入 **Google 日历**会议，会议计为工作时间但**休息自然后延**。
...
- **QQ 音乐联动**：进入休息自动拉起 QQ 音乐并播放；结束休息发暂停媒体键（保留队列/进度）。经系统 Now Playing 路由（CGEvent 媒体键）。
...
- **QQ 音乐**：安装至 `/Applications/QQMusic.app`。其注册为系统 Now Playing 应用，故媒体键可路由控制（**不可** AppleScript 脚本化，已二进制验证）。
- **Google 日历**：在「系统设置 → Internet 账户」添加 Google 账户并启用日历 → 经 CalDAV 同步至 macOS 日历 → EventKit 自动可见（**无 OAuth 代码**，复用 OS 登录态）。

## 配置

配置文件：`~/Library/Application Support/com.aurelius.timeout/config.json`（缺失则用默认）。默认即用户所述作息：
...
- **媒体键竞争**：若其他 app 抢占 Now Playing，媒体键可能路由到错误目标；降级为终止 QQ 音乐保静音。
